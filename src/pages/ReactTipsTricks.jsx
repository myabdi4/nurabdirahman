import Header from "../components/Header";
const ReactTipsTricksBlog = () => {
  return (
    <div>
      <div className="page">
        <Header />
        <div className="sm:flex sm:flex-col text-white pb-5 h-cover items-center">
          <h1 className=" text-4xl p-5 text-center uppercase font-bold">
            The FUn of react
          </h1>
          <div className="flex object-cover pb-5 justify-center">
            <img
              className="w-72 bg-green-400 rounded-lg "
              src="blogs/react.png"
              alt=""
            />
          </div>

          <p className="whitespace-wrap text-2xl p-5 sm:mx-80 tracking-normal text-justify border-t-2 border-gray-800">
            <h2 className="pb-5 text-green-500 font-bold">Why React?</h2>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Praesentium
            suscipit a quam tenetur nobis excepturi nesciunt enim placeat,
            doloremque esse, veritatis ipsum tempore fugit illo, temporibus
            sequi dolores inventore provident. Sed nobis laborum, neque deleniti
            nisi magnam aperiam quia harum, aliquid labore quibusdam in
            inventore, impedit aspernatur non tenetur eum minus consequatur.
            Temporibus labore odit ex consectetur quidem, magni at? Vitae cum
            odit dicta impedit vero modi suscipit? Sed repellendus
            <br />
            <h2 className="pb-1 pt-5 text-green-500 font-bold">
              Areas Why React would succeed
            </h2>
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ex impedit
            illum adipisci voluptatum voluptates tempora, autem laborum eaque,
            laboriosam pariatur corporis modi magni porro voluptas aliquam ipsa
            amet ullam non. Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Reprehenderit vitae ipsam, culpa expedita quam sequi molestiae
            obcaecati totam praesentium dolorum ratione eum modi laboriosam!
            Amet aliquam ullam mollitia debitis non? Lorem ipsum dolor, sit amet
            consectetur adipisicing elit. Ex impedit illum adipisci voluptatum
            voluptates tempora, autem laborum eaque, laboriosam pariatur
            corporis modi magni porro voluptas aliquam ipsa amet ullam non.
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Reprehenderit vitae ipsam, culpa expedita quam sequi molestiae
            obcaecati totam praesentium dolorum ratione eum modi laboriosam!
            Amet aliquam ullam mollitia debitis non? <br /> <br />
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ex impedit
            illum adipisci voluptatum voluptates tempora, autem laborum eaque,
            laboriosam pariatur corporis modi magni porro voluptas aliquam ipsa
            amet ullam non. Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Reprehenderit vitae ipsam, culpa expedita quam sequi molestiae
            obcaecati totam praesentium dolorum ratione eum modi laboriosam!
            Amet aliquam ullam mollitia debitis non?
            <br />
            <h2 className="pb-1 pt-5 text-green-500 font-bold">
              The Struggles are real
            </h2>
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ex impedit
            illum adipisci voluptatum voluptates tempora, autem laborum eaque,
            laboriosam pariatur corporis modi magni porro voluptas aliquam ipsa
            amet ullam non. Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Reprehenderit vitae ipsam, culpa expedita quam sequi molestiae
            obcaecati totam praesentium dolorum ratione eum modi laboriosam!
            Amet aliquam ullam mollitia debitis non?
            <br /> <br />
            <h2 className="pb-1 pt-5 text-green-500 font-bold">In Conlusion</h2>
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Ex impedit
            illum adipisci voluptatum voluptates tempora, autem laborum eaque,
            laboriosam pariatur corporis modi magni porro voluptas aliquam ipsa
            amet ullam non. Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Reprehenderit vitae ipsam, culpa expedita quam sequi molestiae
            obcaecati totam praesentium dolorum ratione eum modi laboriosam!
            Amet aliquam ullam mollitia debitis non?
          </p>
        </div>
      </div>
    </div>
  );
};

export default ReactTipsTricksBlog;
